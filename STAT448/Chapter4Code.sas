...
data hyper;
  * modify path to point to your files;
  infile '/folders/myfolders/data source/hypertension.dat';
  input n1-n12;
...
/* test equal variance and add Welch adjustment as an example */
proc anova data= hyper;
	class drug;
	model bp=drug;
	means drug/ hovtest welch;  /* if reject the hypothesis in hovtest, we should use welch test*/
run;
...
proc anova data=hyper;
  class drug diet biofeed;
  model bp=drug|diet|biofeed;  /* not include the higher interaction if the lower interaction is not significant*/
run;
...
proc anova data=hyper;
  class drug diet biofeed;
  model logbp=drug|diet|biofeed; /* the interaction p value does not have a huge difference, so we donot use scale*/
run;
