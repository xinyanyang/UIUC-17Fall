...
data skulls;
  infile '/folders/myfolders/data source/tibetan.dat' expandtabs;
...
run;
/* obtain the discriminant and classification counts and errors using oen explanatory variable; length */
proc discrim data=skulls out=outresub outd=outdens manova/*option */  ;
   class type/*group variable*/ ;
   var length/*explanatory variables*/ ;
run;  *popular prior probability is 0.5 vs 0.5, can use sample proportions ;

proc print data=outresub;
run; *how the sas assign each observation to group a and b(with greater probability);
...
*in most cases, cv error rate is different from the ordinary error rate, especially if there are more obervatios;
...
run;
/* check :
1)if the classification is meaningful? 
   Multivariate Statistics and Exact F Statistics-- null hypothesis: same mean for all groups
2) LDA or QDA?
   LDA: Test of Homogeneity of Within Covariance Matrices: not reject null:same variance for all groups
3) resubstitution error: 0.1882, use whole dataset for modeling and for testing, tend to overestimate
   cross-validation error: 0.3471, closer to the true test error;


/* use stepwise selection to extract most significant explanatory variables */
proc stepdisc data=skulls sle=0.05 sls=0.05;
	class type;
	var length--facewidth;
	ods select Summary;
run;
