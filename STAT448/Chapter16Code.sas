...
proc corr data=Crime;  * varaiables are highly correlated;
run;
...
1st componet: measure of security because all coefs are similar 
2nd component: large positive magnitude in auto_theft and larcency
               large negative magnitude in murder and assault
               indicator particular crime
3rd component: large positive: indicator of auto_theft and robbery;
...
data decathlon;
  infile '/folders/myfolders/data source/olympic.dat';
...
*three criteria for choose PCs- choose 3 PCs;

*interpret pc- check the eigenvectors
1st- overall preformance
2nd- large positive coef: discus, shot
     large negative coef: run1500, run400, run100
     the contrast between muscle strength and distance running
3rd- measure the Hjump;

proc princomp cov data=decathlon; *use covariance matrix instead of correlation matrix; the diagnoal is not 1 any more ;
	var run100--run1500;
	id name;
run;
*choose 2 PCs using cateria = total variance/10;
*the 1st pc: run1500 larger than the others a lot because the large covariance;
*suggest using correlation matrix because give same weight to every predictor;
...
data pain (type = corr);
	* modify file path to match your files;
	infile '/folders/myfolders/data source/pain.dat' expandtabs missover;
